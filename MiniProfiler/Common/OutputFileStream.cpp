...
namespace CppEssentials
{
    ...
    void OutputFileStream::Open(const wstring & filePath, FileOpenMode eMode)
    {
        ...
        _handle = CreateFileW(filePath.c_str(), GENERIC_WRITE, 0, 0, openMode, FILE_ATTRIBUTE_NORMAL, 0);
        ...
        if (eMode == Append)
        {
            DWORD pos = SetFilePointer(_handle, 0, NULL, FILE_END);
    ...
    void OutputFileStream::Write(byte value)
    {
        ...
        else
        {
            ...
            BOOL result = WriteFile(_handle, (LPVOID)&value, 1, &bytesWritten, (LPOVERLAPPED)0);
    ...
    void OutputFileStream::_WriteChunk(const byte * data, UInt32 length)
    {
        ...
        DWORD bytesWritten = 0;
        BOOL result = WriteFile(_handle, (LPVOID)data, length, &bytesWritten, (LPOVERLAPPED)0);
