...
namespace CppEssentials
{
	...
	{
		_ASSERTE(sizeof(UInt32) >= sizeof(size_t));
		UInt32 length = static_cast<UInt32>(value.length()) * sizeof(wchar_t);
		WriteUInt32(length);
	...
	void BinaryWriter::WriteComString(const _bstr_t& value)
	{
		...
		if (length > 0)
		{
			_outputStream->Write(reinterpret_cast<byte*>(static_cast<wchar_t*>(value)), length);
		}
	}
	...
	{
		if (_hasOwnership && _outputStream != nullptr)
		{
